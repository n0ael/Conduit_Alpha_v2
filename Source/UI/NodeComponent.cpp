      ...
      nodeUuid (nodeTree.getProperty (id::nodeId).toString())
{
    ...
    isLooperPatchInNode     = (factoryKey == LooperPatchInModule::staticModuleId);
    isLooperPatchOutNode = (factoryKey == LooperPatchOutModule::staticModuleId);
    ...
    else if (isLooperPatchInNode)
    {
        looperPatchInPanel = std::make_unique<LooperPatchInPanel> (nodeTree, graphManager);
        addAndMakeVisible (*looperPatchInPanel);

        const auto numSlots = juce::jmax (1, nodeTree.getChildWithName (id::inputs).getNumChildren());
        setSize (280, touchTarget + LooperPatchInPanel::heightForInputs (numSlots));
    }
...
void NodeComponent::beginTeardown()
{
    ...
    if (looperPatchInPanel != nullptr)
        looperPatchInPanel->stopUpdates();


    if (looperPatchOutPanel != nullptr)
        looperPatchOutPanel->stopUpdates();
...
{
    // Slot-Breiten aus dem Tree: Looper patch IN trägt <Inputs>
    // (mono|stereo, Pass-Through — beide Bänke identisch)
    int offset = 0;

    if (isLooperPatchInNode)
    {
    ...
    else if (isLooperPatchOutNode)
    {
        ...
        return channel % LooperPatchOutModule::slotWidth == 0
...
void NodeComponent::rebuildPorts()
{
    ...
    const auto pairStartForBank = [this] (bool isInputBank) -> std::function<bool (int)>
    {
        ...
        if (isLooperPatchInNode
            || (isLooperPatchOutNode && ! isInputBank))
...
int NodeComponent::looperSlotRowFor (int channel) const
{
    ...
    // dieselbe Quelle wie looperSlotPairStart (nur Looper patch IN)
    const auto items = nodeTree.getChildWithName (id::inputs);
    int offset = 0;
    for (int i = 0; i < items.getNumChildren(); ++i)
    {
        const int width = items.getChild (i).getProperty (id::inputMode).toString()
...
int NodeComponent::channelRowY (bool isInputBank, int channel) const
{
    ...
    if (isLooperPatchInNode && looperPatchInPanel != nullptr)
        return looperPatchInPanel->getY() + LooperPatchInPanel::rowCentreY (looperSlotRowFor (channel));
